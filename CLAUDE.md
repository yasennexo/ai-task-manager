...
7. **Write `slack.json`**
   - Assemble the user's Slack user ID, all discovered channels, and DMs into the `slack.json` format (see `slack.example.json` for structure).
...
- "change task X project to nexo" → `npm run cli update <full-id> '{"project":"nexo"}'`
- "change task X priority to high" → `npm run cli update <full-id> '{"priority":"high"}'`
